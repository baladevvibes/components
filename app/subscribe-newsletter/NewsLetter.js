const NewsLetterData = [
  {
    _id: 0,
    name: "Subscribe Newsletter Attraction",
    title: "Subscribe Newsletter Attraction",
    htmlcode: ` <section>
      <div class=" my-40 container mx-auto px-4">
        <div class=" bg-[#0f67f5] overflow-hidden py-10  px-10 rounded-lg relative">
          <div class=" absolute  bg-[#70a1ff] -left-6 -top-4 z-10 h-[100px] w-[100px]  rounded-full"></div>
          <div class=" grid  lg:grid-cols-3 lge:grid-cols-3 md:grid-cols-1 mdsm:grid-cols-1 sm:grid-cols-1">
            <div class=" col-span-2">
              <h2 class=" text-[#fff] z-20  relative concert text-4xl">
                Subscribe Newsletter
              </h2>
              <h2 class="pt-2 catamaran z-20  relative text-[18px] text-[#fff] ">
                Get the latest industry news, trends, and exclusive content
                delivered directly to your inbox.
              </h2>
            </div>
            <div>
              <div class=" relative lg:mt-0 lge:mt-0 md:mt-4 mdsm:mt-4 sm:mt-4">
                <form>
                  <label class="text-[#fff] catamaran">e-mail</label>
                  <input
                    type="text"
                    class="w-full  py-2 text-[#434343] px-3  catamaran outline-none border border-[#fff]"
                    required
                  />
                  <div class=" absolute z-20 group bottom-0 right-0 ">
                    <button class="  group-hover:bg-[#fff] group-hover:text-[#0f67f5] group-hover:border-[#0f67f5] flex space-x-1 text-[#fff] bg-[#0f67f5] border border-[#fff] catamaran px-10 py-2">
                      Subscribe
                      <svg
                        stroke="currentColor"
                        fill="currentColor"
                        stroke-width="0"
                        viewBox="0 0 256 256"
                        class=" mx-1 mt-1"
                        height="1em"
                        width="1em"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M227.57,27.7a7,7,0,0,0-7.13-1.22L17.78,105.79a12.23,12.23,0,0,0,2.1,23.39L74,139.81V200a14,14,0,0,0,24.08,9.71l26.64-27.63,41.58,36.45a13.9,13.9,0,0,0,9.2,3.49,14.33,14.33,0,0,0,4.36-.69,13.86,13.86,0,0,0,9.34-10.17L229.82,34.57A7,7,0,0,0,227.57,27.7ZM22.05,117.37h0a.46.46,0,0,1,0-.32.51.51,0,0,1,.15-.08L181.91,54.45l-103.3,74L22.2,117.41Zm67.39,84A2,2,0,0,1,86,200V148.11l29.69,26Zm88.07,7.08a1.93,1.93,0,0,1-1.34,1.44,2,2,0,0,1-2-.4L89.64,135.34,215,45.5Z"></path>
                      </svg>
                    </button>
                  </div>
                </form>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>`,
  },
];

export default NewsLetterData;
